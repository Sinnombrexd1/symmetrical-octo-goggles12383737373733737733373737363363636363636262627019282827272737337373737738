...
if game.PlaceId ~= 142823291 and game.PlaceId ~= 84724689063885 then 
...
local Window = redzlib:MakeWindow({"CapybaraScript", "BY CapybaritaYT.  v7.5.3", "testando-redzLibv5.json"})
...
local function setTransparencyBasedOnObstacles()
    local player = game.Players.LocalPlayer
    ...
    if character and character:FindFirstChild("Humanoid") then
        ...
        local hits = workspace:FindPartsInRegion3(Region3.new(origin - Vector3.new(5, 5, 5), origin + direction + Vector3.new(5, 5, 5)), nil, math.huge)

        for _, hitPart in pairs(hits) do
            if hitPart:IsDescendantOf(workspace) and not hitPart:IsDescendantOf(character) and hitPart.Transparency ~= 0.7 then
                hitPart.Transparency = 0.7
                transparentParts[hitPart] = true
            end
        end

        for part in pairs(transparentParts) do
            local rayCheck = Ray.new(origin, (part.Position - origin).unit * 10)
            local blockingPart = workspace:FindPartOnRay(rayCheck, character)
...
local function setCameraNoclip()
    local player = game.Players.LocalPlayer
    local camera = game.Workspace.CurrentCamera

    camera.CameraType = Enum.CameraType.Scriptable
    camera.CFrame = camera.CFrame * CFrame.new(0, 0, 0) -- Asegúrate de que la cámara no se vea afectada por la física

    -- Permitir que la cámara atraviese obstáculos
    noclipConnection = game:GetService("RunService").Stepped:Connect(function()
        setTransparencyBasedOnObstacles()
    end)
end
...
Tab1:AddToggle({
    Name = "noclip camara",
    Description = "",
    Default = savedConfig.aims or false,
    Callback = function(state)
        savedConfig.aims = state
...
local function predictPosition(murderer)
    ...
    if humanoidRootPart then
        ...
        local predictionTime = distanceToPlayer > 25 and 0.1 or 0.02  -- Mayor predicción para distancia lejana
        local predictedPosition = murdererPosition + murdererVelocity * predictionTime

        -- Ajuste lateral basado en la dirección de movimiento
        if murdererVelocity.Magnitude > 15 then
            local lateralAdjustment = murdererVelocity.Unit * 0.4  -- Ajuste lateral
            predictedPosition = predictedPosition + lateralAdjustment
        end

        -- Ajuste vertical para apuntar al torso y evitar desviaciones hacia arriba
        return Vector3.new(predictedPosition.X, murdererPosition.Y + -1, predictedPosition.Z) -- Ajusta la altura aquí
    end
    return nil
end
...
shootButton.MouseButton1Click:Connect(function()
    ...
    if gun then
        ...
        if murderer then
            while murderer and murderer.Parent do
                local predictedPosition = predictPosition(murderer)
                if predictedPosition then
                    shootAtMurderer(predictedPosition)
                end
                wait(0.05)  -- Ajusta el tiempo del bucle para precisión
            end
        end
    else
...
function StartKnifeAura()
    ...
    local function equipKnife()
        if not knifeEquipped then
            local tool = player.Backpack:FindFirstChild("Knife")
            if tool then
    ...
    coroutine.wrap(attackNearbyPlayers)()
    ...
    if success and roles then
        for playerName, playerData in pairs(roles) do
            ...
            if player and playerData and playerData.Role then
                ...
                if (role == "Murderer" or role == "Sheriff" or role == "Hero") and not notifiedRoles[player.UserId] then
...
local Toggle = Tab2:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then
            notifiedRoles = {}
            loopConnection = RunService.Heartbeat:Connect(updateRoleNotifications)
        else
...
local function flingKnifeAt(target)
    ...
    local direction = (targetPosition - playerPosition).Unit  -- Dirección desde el jugador local hacia el enemigo

    local knifeArgs = {
        [1] = CFrame.new(playerPosition, playerPosition + direction * 100),  -- Inicia desde el jugador hacia el enemigo
        [2] = targetPosition  -- Objetivo en la posición del enemigo
    }
...
Tab5:AddToggle({
    ...
    Callback = function(toggleState)
        ...
        local function activateSecondLife()
            if secondLifeActivated then return end
...
if CoinCollectedEvent then
    CoinCollectedEvent.OnClientEvent:Connect(function(cointype, current, max)
        -- Se ha eliminado la funcionalidad relacionada con resetWhenFullBag
    end)
end
...
local function anchorCharacterHorizontally(anchor)
    local character = player.Character
    if character then
...
local function OnNewRound(CoinContainer)
    if not running or not CoinContainer then return end

    local Character = player.Character or player.CharacterAdded:Wait()
    local HumanoidRootPart = Character:WaitForChild("HumanoidRootPart")

    while running do
        if not CoinContainer.Parent or not murdererExists() then
            stopAutoFarm()
            break
        end
        ...
        -- Verificar si closestCoin es nil antes de continuar
        if closestCoin then
            if closestDistance > teleportDistance then
                -- Teletransportarse si la moneda está lejos
                HumanoidRootPart.CFrame = CFrame.new(closestCoin.Position - Vector3.new(0, 3, 0))
            else
                -- Anclar el HumanoidRootPart en posición horizontal y mover inmediatamente a la moneda
                anchorCharacterHorizontally(true)
                local targetPosition = closestCoin.Position - Vector3.new(0, 2, 0)
                HumanoidRootPart.CFrame = CFrame.new(targetPosition) * CFrame.Angles(math.rad(90), 0, 0) -- Orientación horizontal

                -- Verificar posición exacta y destruir la moneda
                if (HumanoidRootPart.Position - closestCoin.Position).magnitude < 2 and closestCoin.Parent then
                    closestCoin:Destroy()
                end

                anchorCharacterHorizontally(false)
            end
        end
...
local function stopAutoFarm()
    ...
    -- Desconectar el modo noclip y restaurar colisiones
    if noclipConnection then
        noclipConnection:Disconnect()
        noclipConnection = nil
    end
